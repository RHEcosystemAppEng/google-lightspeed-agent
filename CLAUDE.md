...
1. `auth/middleware.py` validates Bearer tokens on POST `/` (skips `/health`, `/.well-known/agent.json`)
...
├── config/settings.py      # Pydantic BaseSettings (all env vars, validation)
├── core/agent.py           # LlmAgent creation with MCP tools
├── db/                     # SQLAlchemy ORM (5 models, async engine)
...
All configuration is via environment variables, managed through Pydantic settings in `config/settings.py`. See `.env.example` for the full list. Key variables:
...
- `MCP_TRANSPORT_MODE`, `MCP_SERVER_URL` (MCP server connection)
- `RED_HAT_SSO_CLIENT_ID` / `RED_HAT_SSO_CLIENT_SECRET` (auth)
