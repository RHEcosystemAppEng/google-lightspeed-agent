...
Start the Cloud SQL Auth Proxy and fetch database credentials:
...
```bash
PGPASSWORD=$DB_PASSWORD psql -h localhost -U insights -d lightspeed_agent -c "
INSERT INTO marketplace_entitlements (id, account_id, provider_id, state)
VALUES ('$TEST_ORDER_ID', '$TEST_ACCOUNT_ID', 'test-provider', 'active')
...
# Check entitlement
PGPASSWORD=$DB_PASSWORD psql -h localhost -U insights -d lightspeed_agent -c \
  "SELECT id, account_id, state FROM marketplace_entitlements WHERE id = '$TEST_ORDER_ID';"
```
...
Your `RED_HAT_TOKEN` is now ready to use with the A2A endpoint. The middleware
will introspect it, find the `azp` claim matching your seeded DCR client, resolve
the order, and verify the entitlement is active.
...
# Remove entitlement
PGPASSWORD=$DB_PASSWORD psql -h localhost -U insights -d lightspeed_agent -c \
  "DELETE FROM marketplace_entitlements WHERE id = '$TEST_ORDER_ID';"
```
...
PGPASSWORD=$DB_PASSWORD psql -h localhost -U insights -d lightspeed_agent -c \
  "SELECT client_id, order_id FROM dcr_clients WHERE client_id = '<azp-value>';"

# Check if the entitlement exists and is active
PGPASSWORD=$DB_PASSWORD psql -h localhost -U insights -d lightspeed_agent -c \
  "SELECT id, state FROM marketplace_entitlements WHERE id = '<order-id>';"
```
